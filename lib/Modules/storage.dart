...
void deleteFromStorageModule(VideoIndexModel video,
  ...
  try {
    ...
    ref.delete();
    ref = storage.ref().child(video.directory.replaceFirst(
        "/", "/thumbnail_", video.directory.lastIndexOf("/") - 1));
    ref.delete();
    GlobalClass.userDetail.cloudStorageSize =
        GlobalClass.userDetail.cloudStorageSize -
            video.thumbnailSize -
            video.cloudStorageSize;
...
Future<void> handleTask1(
    GlobalKey<ScaffoldMessengerState> scaffoldMessengerKey) async {
  firebase_storage.UploadTask task =
      storage.ref('uploads/hello-world.txt').putString('Hello World');

  try {
    // Storage tasks function as a Delegating Future so we can await them.
    firebase_storage.TaskSnapshot snapshot = await task;
    globalShowInSnackBar(
        scaffoldMessengerKey, 'Uploaded ${snapshot.bytesTransferred} bytes.');
  } on firebase_core.FirebaseException catch (e) {
...
Future<void> handleTask2(String filePath,
    GlobalKey<ScaffoldMessengerState> scaffoldMessengerKey) async {
  File largeFile = File(filePath);

  firebase_storage.UploadTask task =
      storage.ref('uploads/hello-world.txt').putFile(largeFile);
...
Future<void> handleTask4(String filePath,
    GlobalKey<ScaffoldMessengerState> scaffoldMessengerKey) async {
  File largeFile = File(filePath);
  ...
  task.snapshotEvents.listen((firebase_storage.TaskSnapshot snapshot) {
    ...
    globalShowInSnackBar(
        scaffoldMessengerKey, firebase_storage.TaskState.error.toString());
  });
