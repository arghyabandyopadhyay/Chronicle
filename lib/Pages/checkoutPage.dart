...
class CheckoutPage extends StatefulWidget {
  ...
  const CheckoutPage({Key key, this.courses, this.isFromPreviewPage})
      : super(key: key);
  @override
  ...
  void openCheckout() {
    if (widget.courses != null) {
      var options = {
        ...
        "description":
            "Courses purchased by ${GlobalClass.userDetail.displayName}",
  ...
  Future<void> handlerPaymentSuccess(PaymentSuccessResponse response) async {
    ...
    final invoice = Invoice(
        ...
        info: InvoiceInfo(
          ...
          number:
              '${DateTime.now().year}-${DateTime.now().millisecondsSinceEpoch}',
        ),
    ...
    if (widget.isFromPreviewPage) {
      List<CourseIndexModel> temp = GlobalClass.myCourses
          .where((element) => element.uid == widget.courses[0].uid)
          .toList();
      if (temp.length == 0)
        addCoursesToOwnLists("Courses", widget.courses[0]);
      else {
