...
import 'package:chronicle/Widgets/video_list.dart';
...
class _VideosPageState extends State<VideosPage> {
  ...
  void _handleSearchEnd() {
    setState(() {
      ...
      this.appBarTitle = Text(widget.isPreview
          ? "Select Preview Video"
          : widget.isAddVideoToCourse
              ? "Add Videos"
              : "Videos");
      _isSearching = false;
  ...
  void searchOperation(String searchText) {
    ...
    if (_isSearching) {
      searchResult = videos
          .where((VideoIndexModel element) => (element.masterFilter).contains(
        ...
        leading: IconButton(
          onPressed: () {
            if (_isSearching) {
              ...
              if (widget.isAddVideoToCourse) {
                if (selectedList.length == 0)
                  ...
                  scaffoldMessengerKey.currentState.showSnackBar(new SnackBar(
                    content: Text(
                        "Press add button to add the selected videos to the course."),
              ...
              scaffoldMessengerKey.currentState.hideCurrentSnackBar();
              scaffoldMessengerKey.currentState.showSnackBar(new SnackBar(
                content: Text("Video is being uploaded."),
                action: SnackBarAction(
                  label: "Leave anyway",
                  textColor: CustomColors.snackBarActionTextColor,
                  onPressed: () {
                ...
                ModalOptionModel(
                    ...
                    onTap: () async {
                      ...
                      Navigator.of(context).push(new CupertinoPageRoute(
                          builder: (context) =>
                              AddVideoPage(callback: this.uploadFile)));
                    }),
              ...
              onPressed: () async {
                showDialog(
                    ...
                    builder: (_) => new AlertDialog(
                          ...
                          content: Text(
                              "Are you sure to add all the selected videos?"),
                          actions: [
                            ActionChip(
                                label: Text("Yes"),
                                onPressed: () {
            ...
            onPressed: () async {
              showDialog(
                  ...
                  builder: (_) => new AlertDialog(
                        ...
                        content: Text(
                            "Are you sure to delete all the selected videos?\n The change is irreversible."),
                        actions: [
                          ActionChip(
                              label: Text("Yes"),
                              onPressed: () {
                                setState(() {
                                  selectedList.forEach((element) {
                                    deleteFromStorageModule(
                                        element, scaffoldMessengerKey);
                                    if (_isSearching)
  ...
  Future<void> uploadFile(String filePath, String thumbnailFilePath,
      String name, String description, String masterFilter) async {
    try {
      progressValue = 0;
      ...
      firebase_storage.UploadTask task = storage
          .ref('${GlobalClass.user.uid}/$today/$storageId')
          .putFile(file,
      ...
      task.snapshotEvents.listen((firebase_storage.TaskSnapshot snapshot) {
        ...
        if (e.code == 'permission-denied') {
          globalShowInSnackBar(scaffoldMessengerKey,
              'You does not have permission to upload to this reference.');
        }
      });
      await task;
      globalShowInSnackBar(scaffoldMessengerKey,
          "The video has been uploaded. Uploading metadata...");
      String url =
          await downloadURL('${GlobalClass.user.uid}/$today/$storageId');
      if (thumbnailFilePath == null || thumbnailFilePath == "")
        thumbnailFilePath = await VideoThumbnail.thumbnailFile(
          ...
          maxWidth:
              1000, // specify the height of the thumbnail, let the width auto-scaled to keep the source aspect ratio
          quality: 75,
        );
      File thumbnailFile = File(thumbnailFilePath);
      int thumbnailFileSize = await thumbnailFile.length();
      firebase_storage.UploadTask task2 = storage
          .ref('${GlobalClass.user.uid}/$today/thumbnail_$storageId')
          .putFile(thumbnailFile,
              firebase_storage.SettableMetadata(contentType: 'WEBP'));
      ...
      await task2;
      String thumbnailUrl = await downloadURL(
          '${GlobalClass.user.uid}/$today/thumbnail_$storageId');
      newVideoIndexModel(new VideoIndexModel(
          cloudStorageSize: fileSize,
          thumbnailSize: thumbnailFileSize,
          name: name,
          description: description,
          masterFilter: masterFilter,
          directory: '${GlobalClass.user.uid}/$today/$storageId',
          downloadUrl: url,
          thumbnailUrl: thumbnailUrl));
      GlobalClass.userDetail.cloudStorageSize =
          GlobalClass.userDetail.cloudStorageSize +
              thumbnailFileSize +
              fileSize;
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: ScaffoldMessenger(
        child: Scaffold(
          appBar: getAppBar(),
          body: this.videos != null
              ? this.videos.length == 0
                  ? NoDataError()
                  : Column(children: <Widget>[
                      Center(
                        child: Text(
                          "Storage Occupied: ${classifySize(GlobalClass.userDetail.cloudStorageSize)} of ${classifySize(GlobalClass.userDetail.cloudStorageSizeLimit)}",
                          style: TextStyle(
                              color: GlobalClass.userDetail.cloudStorageSize >
                                      GlobalClass
                                          .userDetail.cloudStorageSizeLimit
                                  ? Colors.red
                                  : null),
                        ),
                      ),
