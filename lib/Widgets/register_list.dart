...
import 'package:chronicle/Pages/MasterPages/chronicle_master_page.dart';
...
class RegisterList extends StatefulWidget {
  ...
  RegisterList(this.isDialog, this.isAddToRegister, this.selectedClients,
      this.scaffoldMessengerKey, this.mainScreenContext);
  @override
...
class _RegisterListState extends State<RegisterList> {
  ...
  Widget build(BuildContext context) {
    return StaggeredGridView.countBuilder(
      ...
      itemBuilder: (BuildContext staggeredGridViewContext, int index) =>
          GestureDetector(
        child: Container(
            color: Colors.grey.withOpacity(0.1),
            alignment: Alignment.center,
            child: Text(
              GlobalClass.registerList[index].name,
              textAlign: TextAlign.center,
              style: TextStyle(fontSize: 30, fontWeight: FontWeight.bold),
            )),
        onTap: () {
          if (widget.isAddToRegister) {
            if (GlobalClass.registerList[index].id.key ==
                GlobalClass.lastRegister) {
              globalShowInSnackBar(widget.scaffoldMessengerKey,
                  "You can't add to the same register!!");
            } else {
                  ...
                  builder: (_) => OptionModalBottomSheet(
                        ...
                        list: [
                          ModalOptionModel(
                              ...
                              onTap: () {
                                moveClientsModule(widget.selectedClients,
                                    GlobalClass.registerList[index]);
                          ...
                          ModalOptionModel(
                              ...
                              icon: Icons.copy,
                              onTap: () {
                                copyClientsModule(widget.selectedClients,
                                    GlobalClass.registerList[index]);
                                Navigator.of(context).pop();
                                Navigator.of(_).pop();
                              }),
            ...
            Navigator.of(widget.mainScreenContext != null
                    ? widget.mainScreenContext
                    : context)
                .pushReplacement(CupertinoPageRoute(
                    builder: (context) => ChronicleMasterPage(
                        register: GlobalClass.registerList[index])));
        ...
        onLongPress: () {
          showDialog(
              ...
              builder: (_) => new AlertDialog(
                    ...
                    content: Text(
                        "Are you sure to delete \"${GlobalClass.registerList[index].name}\" ? All the client data will be deleted."),
                    actions: [
                      ActionChip(
                          label: Text("Yes"),
                          onPressed: () {
                            setState(() {
                              deleteDatabaseNode(
                                  GlobalClass.registerList[index].id);
                              deleteDatabaseNode(getDatabaseReference(
                                  "registers/${GlobalClass.registerList[index].uid}"));
                              if (GlobalClass.lastRegister != null &&
                                  GlobalClass.lastRegister ==
                                      GlobalClass.registerList[index].id.key)
                                setLastRegister("");
        ...
        onDoubleTap: () {
          if (widget.isDialog) {
            if (GlobalClass.registerList[index].id.key ==
                GlobalClass.lastRegister) {
              globalShowInSnackBar(widget.scaffoldMessengerKey,
                  "You can't rename your current register here!!");
            } else
              showDialog(
                  context: context,
                  builder: (_) => new AlertDialog(
                        ...
                        content: TextField(
                          controller: renameRegisterTextEditingController,
                          ...
                          ActionChip(
                              ...
                              onPressed: () {
                                if (renameRegisterTextEditingController.text !=
                                    "") {
                                  GlobalClass.registerList[index].name =
                                      renameRegisterTextEditingController.text
                                          .replaceAll(
                                              new RegExp(r'[^\s\w]+'), "");
                                  renameRegister(
                                      GlobalClass.registerList[index],
